...
% Making an energy axis
h_int = 1e-21;
E = h*c/(wav(end)*1e-9):h_int:h*c/(wav(1)*1e-9);
...
for i = 1:length(n_KK_E)
    ...
    j = 1:length(n_KK_E);
    j = j(j~=i);
    int1 = 2/pi*h_int*sum(E(j).*k_E(j)./(E(j).^2-E(i)^2));

    int2 = 1/pi*h_int*k_E(i)/(2*E(i));
    if i == 1
        int3 = 1/pi*h_int*(k_E(i+1)-k_E(i))/h_int;
    elseif i == length(n_KK_E)
        int3 = 1/pi*h_int*(k_E(i)-k_E(i-1))/h_int;
    else
        int3 = 1/pi*h_int*(k_E(i+1)-k_E(i-1))/(2*h_int);
    end
